...
__relationship_entityids = set()

def relationship_entities(api):
    global __relationship_entities
    global __relationship_entityids
    ...
    for entities in api.entity_id_mapping_by_relationship_name_list_2(names):
        ...
        if len(entities.ids) > 1:
            ...
            entitiesFrom = entities.ids[1:]
            entityTo = entities.ids[0]
            log.warning("More than one entity for relationship '%s' found: %s %s will be merged into entity %i"
                % (entities.name, (len(entitiesFrom) == 1 and 'entity') or 'entities', ','.join(str(id) for id in entitiesFrom), entityTo))
            # autoheal: do auto merging here
            for entityFrom in entitiesFrom:
                log.info('merging entity %i into entity %i' % (entityFrom, entityTo))
                api.mergeEntities(entityFrom, entityTo, commit=False)
        __relationship_entities[entities.name] = entities.ids[0]
        # __relationship_entityids |= set(entities.ids)
        __relationship_entityids.add(entities.ids[0])
    if merged:
        api.commit()
...
def build_relations(api, entity_a, frames, mentioned_entities):
    ...
    relationshipNames = set(relationship_entities(api).keys())
    ...
    for frame in frames:
        if frame.get('FrameType') == 3:
            ...
            # failsafe, no relationship entities allowed as partners !
            if partner1ID in __relationship_entityids or partner2ID in __relationship_entityids:
                continue
            ...
            if next_relations:
                ...
                for frame_b in frames_b:
                    if frame_b.get('FrameType') == 3:
                        ...
                        partner1ID = None
                        partner2ID = None
                        for fd in frame_b.get('FrameData', tuple()):
                            if fd.get('Key') == 4:
                        ...
                        if entity_a in __relationship_entityids or entity_c in __relationship_entityids:
                            # NOTE: this is the cause of all problems; FIXME: this is workaround, code must be fixed - this shouldn't be happening
                            continue
